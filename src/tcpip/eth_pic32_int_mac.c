...
bool MACIsTxReady(void) {
    ...
    if (_pTxCurrDcpt == 0) {
        for (ix = _TxLastDcptIx + 1; ix<sizeof (_TxDescriptors) / sizeof (*_TxDescriptors); ix++) {
            if (_TxDescriptors[ix].txBusy == 0) { // found a non busy descriptor
...
void MACDiscardRx(void) {
    if (_pRxCurrBuff) { // an already existing packet
...
bool MACGetHeader(MAC_ADDR *remote, BYTE* type) {
    ...
    if (res == ETH_RES_OK) { // available packet; minimum check

        if (pRxPktStat->rxOk && !pRxPktStat->runtPkt && !pRxPktStat->crcError) { // valid packet;
    ...
    if (_pRxCurrBuff == 0 && pNewPkt) { // failed packet, discard
...
//    if (linkStat & ETH_LINK_ST_UP) { // negotiation succeeded; properly update the MAC
